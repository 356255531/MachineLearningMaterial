...
%
% 2.  Based on the assumptiom that the selected features are independent the following observations
%
...
% load data
observations = importdata('exercise3Observations.txt', ',', 1);
...
% create histograms for headache
figure;
for id=unique(data(:,1))'
    subplot(2,2,id)
    hist(data(data(:,1)==id,6), 32);
    title(['Headache levels for disease with id ' num2str(id)]);
end

% create histograms for tiredness. Compare results
figure;
for id=unique(data(:,1))'
    subplot(2,2,id)
    hist(data(data(:,1)==id,5), 32);
    title(['Tiredness levels for disease with id ' num2str(id)]);
end
